...
// function hosting, the start() method is callable on line 6
...
function start() {
    ...
    // Our bar chart is going to encode the sub_field frequency as bar width.
    ...
    var yScale = d3.scaleBand().rangeRound([0, height-50], 0.3);
    ...
    //Creates the selection menu for selecting a filter color
    d3.select(graph)
    	.append('p')
    	.attr('id', "selections")
    	.text("Select a Color:")
	...
	//Creates the input box for setting the cutoff
	d3.select(graph)
		.append('p')
		.text("Cutoff: ")
		.append('input')
		.attr("name", "cutoff")
		.attr('id', "cutoff-option")
    ...
    d3.select(graph)
        ...
        .on('click', function() {
            bars.selectAll('.bar')
                .filter(function(d) {
                	var cutoff = document.getElementById("cutoff-option").value;
    ...
    //Creates the reset filter button
    d3.select("#buttons")
		.append('button')
            .style("border", "1px solid black")
        .text('Reset Graph')
    ...
    d3.csv('data_subfield.csv', function(d) {
        ...
        bars
          ...
          .attr("height", function (d) {
            ...
            // to contain all the sub_fields in the alphabet?
